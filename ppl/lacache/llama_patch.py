    ...
    **kwargs,
    ...
    if past_key_value is not None:
        if not hasattr(self, "cos"):
            max_position_ids = torch.arange(self.rotary_emb.original_max_seq_len, device=value_states.device).unsqueeze(0)
            self.cos, self.sin = self.rotary_emb(value_states, max_position_ids)
        cos_key, sin_key = self.cos[:, :key_states.shape[2]], self.sin[:, :key_states.shape[2]]
...
def llama_pos_shift_flashattention_forward(
    self,
    hidden_states: torch.Tensor,
...
def enable_llama_pos_shift_attention(model):
    for name, module in reversed(model._modules.items()):
        ...
        if isinstance(module, LlamaFlashAttention2):
            model._modules[name].forward = types.MethodType(
                llama_pos_shift_flashattention_forward, model._modules[name]
            )
